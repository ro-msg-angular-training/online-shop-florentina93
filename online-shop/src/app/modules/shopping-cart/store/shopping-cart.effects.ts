...
export class ShoppingCartEffects {
  ...
  getCartItems = this.actions$.pipe(
    ofType(ShoppingCartActions.GET_CART_ITEMS),
    switchMap(() => this.cartService.getCartItems()),
    map(cartItems => new ShoppingCartActions.GetCartItemsSuccess({cartItems }))
  );
  ...
  postOrder = this.actions$.pipe(
    ...
    map(() => new ShoppingCartActions.CheckoutCartItemsSuccess()),
    catchError(error => {
